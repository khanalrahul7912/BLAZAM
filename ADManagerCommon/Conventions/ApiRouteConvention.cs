...
namespace ADManager.Common.Conventions
{
    /// <summary>
    /// A route convention that modifies controller route templates to include subdirectory paths based on the
    /// controller's namespace structure, but only for controllers under the ADManager.Pages.API namespace.
    /// </summary>
    ...
    /// controllers under ADManager.Pages.API. For example, if a controller resides in the namespace
    /// "ADManager.Pages.API.v1.Subdirectory.Controller", the resulting route template will include "api/v1/Subdirectory"
    ...
    public class ApiRouteConvention : IApplicationModelConvention
    {
        public void Apply(ApplicationModel application)
        {
            foreach (var controller in application.Controllers)
            {
                ...
                // Only affect controllers under ADManager.Pages.API
                if (controllerNamespace == null || !controllerNamespace.StartsWith("ADManager.Pages.API"))
                {
                    continue;
                }
                ...
                {
                    // Join all segments after "ADManager.Pages.API" as subdirectory path
