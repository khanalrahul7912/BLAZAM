#Name ADManager Updater Script
#Purpose Provides a decoupled way to elevate, and update web app
#Version 1.2.0

Write-Host("Performs a self update of the ADManager Web Application.")
...
function StopApp {
    ...
    try {
        Stop-Service -Name "AD Manager" -Force -ErrorAction Stop
        Write-Host("AD Manager service stopped successfully.")
    } catch {
        Write-Host("Warning: Failed to stop AD Manager service - " + $_.Exception.Message)
...
function StartApp {
    ...
    Start-Service -Name "AD Manager" -ErrorAction Continue
...
function PerformBackup {
    ...
    $backupDirectory = $env:TEMP + "\ADManager\backup\" + $backupDateStr + "\"
