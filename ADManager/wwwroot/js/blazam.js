...
window.admanager = {
    ...
    startPolling: () => {
        if (window.admanager.getPushNotificationSubscriptionState() && !window.admanager.pollingInterval) {
            window.admanager.pollingInterval = setInterval(window.admanager.pollForNotifications, 30000);
        }
    },

    stopPolling: () => {
        if (window.admanager.pollingInterval) {
            clearInterval(window.admanager.pollingInterval);
            window.admanager.pollingInterval = null;
        }
    },
    ...
    pollForNotifications: async () => {
        ...
        if (response.ok) {
            ...
            if (notifications && notifications.length > 0) {
                ...
                if (latestNotification.id > window.admanager.lastNotificationId) {
                    navigator.serviceWorker.ready.then((registration) => {
                        registration.showNotification(latestNotification.title || "Notification", {
                            ...
                            tag: latestNotification.tag || "admanager-notification",
                        });
                    });
                }
                window.admanager.lastNotificationId = latestNotification.id;
            }
        }
    }
};
